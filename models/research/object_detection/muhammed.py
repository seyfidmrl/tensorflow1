...
with detection_graph.as_default():
    with tf.Session(graph=detection_graph) as sess:
        while True:
            ...
            isim = (category_index.get(value)).get('name').encode('utf8')            
            name = str(isim)
           

            if name == "b'insan'" and time.time() - start > 5 and bool(objects):
                img_name = "Hedef.jpeg"
                cv2.imwrite(img_name, image_np)
                start = time.time()
                bot.send_message(chatid, "Tanımlanan Obje Tespit Edildi.")
